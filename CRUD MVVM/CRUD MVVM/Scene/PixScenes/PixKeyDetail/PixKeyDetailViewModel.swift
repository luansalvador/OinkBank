...
final class PixKeyDetailViewModel {
    ...
    func verifyRegisterKey() -> Bool{
        if editPixIndex == 0 && pixKey == "Cadastrar Nova Chave"{
            displayAlert(title: "Erro", message: "Chave CPF não possui cadastro".localized(tableName: "LocalizableMyKeys"))
            return false
        }
        
        if editPixIndex == 1 && pixKey == "Cadastrar Nova Chave"{
            displayAlert(title: "Erro", message: "Chave aleatória não possui cadastro".localized(tableName: "LocalizableMyKeys"))
            return false
        }
        
        if editPixIndex == 2 && pixKey == "Cadastrar Nova Chave"{
            displayAlert(title: "Erro", message: "Chave telefone não possui cadastro".localized(tableName: "LocalizableMyKeys"))
            return false
        }
        
        if editPixIndex == 3 && pixKey == "Cadastrar Nova Chave"{
            displayAlert(title: "Erro", message: "Chave e-mail não possui cadastro".localized(tableName: "LocalizableMyKeys"))
            return false
        }
